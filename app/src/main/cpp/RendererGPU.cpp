...
// -----------------------------------------------------------------------------
// File-level logging utility
static void loge(const char* fmt, ...) {
    va_list vl; va_start(vl, fmt);
    __android_log_vprint(ANDROID_LOG_ERROR, "NCAM", fmt, vl);
    va_end(vl);
}

// -----------------------------------------------------------------------------
// File-scope callback functions required by ACameraDevice_StateCallbacks
...
bool NativeCamera::open(int w, int h, FrameCallback cb) {
    ...
    if (ACameraManager_openCamera(mgr_, camIdStr.c_str(), &callbacks, &device_) != ACAMERA_OK) {
        LOGE("Failed to open camera.");
        return false;
    }

    // Create AImageReader once
    if (AImageReader_new(w, h, AIMAGE_FORMAT_YUV_420_888, 4, &reader_) != AMEDIA_OK) {
        LOGE("AImageReader creation failed.");
        return false;
    }
    ...
    // Wait for native window (loop, but not recreate reader/camera)
    ANativeWindow* wnd = nullptr;
    const int maxWaitMs = 2000;
    int waitedMs = 0;
    while ((AImageReader_getWindow(reader_, &wnd) != AMEDIA_OK || !wnd) && waitedMs < maxWaitMs) {
        usleep(10000);  // 10ms
        waitedMs += 10;
    }

    if (!wnd) {
        LOGE("Timeout waiting for ImageReader native window");
        return false;
    }
    ...
    ACaptureSessionOutputContainer_add(outputs, output_);
    ...
    if (ACameraDevice_createCaptureSession(device_, outputs, &sessionCallbacks, &session_) != ACAMERA_OK) {
        LOGE("Capture session creation failed.");
        return false;
    }

    int seq = 0;
    if (ACameraCaptureSession_setRepeatingRequest(session_, nullptr, 1, &req_, &seq) != ACAMERA_OK) {
        LOGE("Repeating request failed.");
        return false;
    }

    LOGI("Camera open complete. Streaming started.");
    return true;
}
...
void NativeCamera::onImage(void* ctx, AImageReader* reader) {
    LOGI("onImage called");  // ← REQUIRED

    NativeCamera* self = static_cast<NativeCamera*>(ctx);
    AImage* img = nullptr;
    if (AImageReader_acquireLatestImage(reader, &img) == ACAMERA_OK && img) {
        self->frameCb_(img);
        AImage_delete(img);
    }
}
...
void NativeCamera::close() {
    // TODO: Cleanup camera handles, delete image reader, etc.
}
